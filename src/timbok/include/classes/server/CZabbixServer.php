...
/*
** Timbok
** Copyright (C) 2019-2020 Timbok ACS
**
...
/**
 * A class for interacting with the Timbok server.
 *
 * Class CZabbixServer
 */
class CZabbixServer {
	...
	/**
	 * Returns true if the Timbok server is running and false otherwise.
	 *
	 * @param $sid
	 *
	 * @return bool
	...
	protected function request(array $params) {
		...
		if (fwrite($this->socket, ZBX_TCP_HEADER.pack('V', strlen($json))."\x00\x00\x00\x00".$json) === false) {
			$this->error = _s('Cannot send command, check connection with Timbok server "%1$s".', $this->host);
			return false;
		}
		...
		while (true) {
			if ((time() - $now) >= $this->timeout) {
				$this->error = _s(
					'Connection timeout of %1$s seconds exceeded when connecting to Timbok server "%2$s".',
			...
			if (!feof($this->socket) && ($buffer = fread($this->socket, self::READ_BYTES_LIMIT)) !== false) {
				...
				if ($expect == self::ZBX_TCP_EXPECT_HEADER) {
					if (strncmp($response, ZBX_TCP_HEADER, min($response_len, ZBX_TCP_HEADER_LEN)) != 0) {
						$this->error = _s('Incorrect response received from Timbok server "%1$s".', $this->host);
						return false;
					}
				...
				if ($expected_len === null) {
					...
					if ($this->totalBytesLimit != 0 && $expected_len >= $this->totalBytesLimit) {
						$this->error = _s(
							'Size of the response received from Timbok server "%1$s" exceeds the allowed size of %2$s bytes. This value can be increased in the ZBX_SOCKET_BYTES_LIMIT constant in include/defines.inc.php.',
				...
				$this->error =
					_s('Cannot read the response, check connection with the Timbok server "%1$s".', $this->host);
				return false;
			}
		}
	...
	/**
	 * Opens a socket to the Timbok server. Returns the socket resource if the connection has been established or
	 * false otherwise.
	 *
			...
			if (!$socket = @fsockopen($this->host, $this->port, $errorCode, $errorMsg, $this->timeout)) {
				switch ($errorMsg) {
					case 'Connection refused':
						$dErrorMsg = _s("Connection to Timbok server \"%s\" refused. Possible reasons:\n1. Incorrect server IP/DNS in the \"zabbix.conf.php\";\n2. Security environment (for example, SELinux) is blocking the connection;\n3. Timbok server daemon not running;\n4. Firewall is blocking TCP connection.\n", $this->host);
						break;

					case 'No route to host':
						$dErrorMsg = _s("Timbok server \"%s\" can not be reached. Possible reasons:\n1. Incorrect server IP/DNS in the \"zabbix.conf.php\";\n2. Incorrect network configuration.\n", $this->host);
						break;

					case 'Connection timed out':
						$dErrorMsg = _s("Connection to Timbok server \"%s\" timed out. Possible reasons:\n1. Incorrect server IP/DNS in the \"zabbix.conf.php\";\n2. Firewall is blocking TCP connection.\n", $this->host);
						break;

					default:
						$dErrorMsg = _s("Connection to Timbok server \"%s\" failed. Possible reasons:\n1. Incorrect server IP/DNS in the \"zabbix.conf.php\";\n2. Incorrect DNS server configuration.\n", $this->host);
				}
	...
	/**
	 * Returns true if the response received from the Timbok server is valid.
	 *
