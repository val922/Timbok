...
class ZBase {
	...
	/**
	 * @var array of config data from Timbok config file
	 */
	protected $config = [];
	...
	protected function initLocales(array $user_data) {
		...
		if (function_exists('bindtextdomain')) {
			...
			if (!$locale_found && $user_data['lang'] != 'en_GB' && $user_data['lang'] != 'en_gb') {
				error('Locale for language "'.$user_data['lang'].'" is not found on the web server. Tried to set: '.implode(', ', $locales).'. Unable to translate Timbok interface.');
			}
