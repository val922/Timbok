...
class CSetupWizard extends CForm {
	...
	function stage0() {
		...
		$setup_title = (new CDiv([new CSpan(_('Welcome to')), 'Timbok '.$version[0]]))->addClass(ZBX_STYLE_SETUP_TITLE);
	...
	function stage3() {
		...
		return [
			new CTag('h1', true, _('Timbok server details')),
			(new CDiv([
				new CTag('p', true, _('Please enter the host name or host IP address and port number of the Timbok server, as well as the name of the installation (optional).')),
				$table
			]))->addClass(ZBX_STYLE_SETUP_RIGHT_BODY)
		];
	}
	...
	function stage4() {
		...
		$table->addRow((new CSpan(_('Timbok server')))->addClass(ZBX_STYLE_GREY), $this->getConfig('ZBX_SERVER'));
		$table->addRow((new CSpan(_('Timbok server port')))->addClass(ZBX_STYLE_GREY), $this->getConfig('ZBX_SERVER_PORT'));
		$table->addRow((new CSpan(_('Timbok server name')))->addClass(ZBX_STYLE_GREY), $this->getConfig('ZBX_SERVER_NAME'));
	...
	function stage5() {
		...
		if ($error) {
			...
			$message_box = null;
			$message = [
				(new CTag('h1', true, _('Congratulations! You have successfully installed Timbok frontend.')))
