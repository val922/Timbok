...
class PostgresqlDbBackend extends DbBackend {
	...
	protected function checkDbVersionTable() {
		...
		if (!$table_exists) {
			$this->setError(_s('Unable to determine current Timbok database version: %1$s.',
	...
	protected function checkDatabaseEncoding(array $DB) {
		...
		if ($row && $row['db_charset'] != ZBX_DB_DEFAULT_CHARSET) {
			$this->setWarning(_s('Incorrect default charset for Timbok database: %1$s.',
	...
	protected function checkTablesEncoding(array $DB) {
		...
		/**
		 * Getting all fields in all Timbok tables to check for collation.
