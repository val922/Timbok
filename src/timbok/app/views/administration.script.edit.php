...
$scriptFormList = (new CFormList())
	...
	->addRow((new CLabel(_('Execute on'), 'execute_on')),
		(new CRadioButtonList('execute_on', (int) $data['execute_on']))
			->addValue(_('Timbok agent'), ZBX_SCRIPT_EXECUTE_ON_AGENT)
			->addValue(_('Timbok server (proxy)'), ZBX_SCRIPT_EXECUTE_ON_PROXY)
			->addValue(_('Timbok server'), ZBX_SCRIPT_EXECUTE_ON_SERVER)
			->setModern(true)
	)
